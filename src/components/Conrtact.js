...
export default function Contact() {
    ...
    function verify(n) {
        var j = 0;
        for (var i = n - 1; i > 0; i--) {
            var ele = document.getElementsByClassName(i)[0]
            if (ele.value === "") {
        ...
        if (j === 1) {
            var doc = document.getElementsByClassName(n)[0]
    ...
    function change(n) {
        if (n !== 3)
            document.getElementsByClassName(n)[0].removeAttribute("style")
        else {
                ...
                <div className="form-reseau">
                    ...
                    <Form className="form" onSubmit={submit}>
                        <Form.Group className="mb-3" controlId="formBasicText" >
                            ...
                            <Form.Control type="text" placeholder="Enter Fullname" className="1" onChange={() => { change(1) }} onClick={() => { verify(1) }} />
                        </Form.Group>
                        <Form.Group className="mb-3" controlId="formBasicEmail">
                            <Form.Label>Email address</Form.Label>
                            <Form.Control type="email" placeholder="Enter email" className="2" onChange={() => { change(2) }} onClick={() => { verify(2) }} />
                        </Form.Group>
                        <Form.Group className="mb-3" controlId="formBasicText">
                            <Form.Label>Message</Form.Label><br />
                            <textarea className="message 3" placeholder="Message" onChange={() => { change(3) }} onClick={() => { verify(3) }}></textarea>
                        </Form.Group>
